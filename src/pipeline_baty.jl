...
function run_pipeline(patterns::Vector{String}, penalties::Vector{Int}, instances::Vector{String}, settings::baty_settings)
    ...
    # Create folder
    models_folder = "training/models/baty/"
    solutions_folder = "training/solutions/baty/"
    ...
    for idx in keys(instance_dict)
        ...
        for s in ((length(samples_dict[idx])/look_ahead)+1):nb_scenarios
            ...
            for p in 1:look_ahead
                if nb_features>0
                    contextual_features = Dict((k-p+1) => Dict(i => vec(convert(Vector{Float64}, values(roll_demand_samples[i][k]["features"]))) for i in indices_dict[idx].V_cus) for k in p:p+look_ahead-1)
                else
